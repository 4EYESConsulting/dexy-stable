...
similar to ErgoDex \cite{ErgoDex} and UniSwap \cite{Uniswap}. This means that the liqudity pools holds a pair of 
...
\State $(\tilde{t}_{\textrm{ED}}, \tilde{t}_{\textrm{RD}}, \tilde{d}, \tilde{\tau}_{\textrm{ED}}, \tilde{\tau_{\textrm{RD}}}) \gets \sburn$
\State $(\Pdt, F) \gets \soracle$ 
\State $\rr \gets \rr(\Pdt, \slp)$
\State $notUpdated \gets (\tau == \infty)$
\State $validUpdate \gets (t' \ge t - \tautx) \land (t' \le t)$ \Comment{Update time is in a valid interval.}
...
\If{$isUpdate \lor isReset$}
  \State $\sburn \gets (\tilde{t}_{\textrm{ED}}, \tilde{t}_{\textrm{RD}}, \tilde{d}, t', \tilde{\tau_{\textrm{RD}}})$
  \State \Return \textsc{true}
\Else
  \State \Return \textsc{false}
\EndIf
...
\If{$isUpdate \lor isReset$}
  \State $\sburn \gets (\tilde{t}_{\textrm{ED}}, \tilde{t}_{\textrm{RD}}, \tilde{d}, \tilde{\tau}_{\textrm{ED}}, t')$
  \State \Return \textsc{true}
\Else
  \State \Return \textsc{false}
\EndIf
...
\begin{algorithmic}[1]
  ...
  \State $\delta_{d} \gets \alpha_{\textrm{FREE}} * D$ \Comment{Represents the maximum bank reserve growth during the FreeMint period.}
  ...
  \Else 
    \If{$T' \in [t + T_{\textrm{FREE}}, t + T_{\textrm{FREE}} + \tautx]$} \Comment{Checks new FreeMint period end time.}
      \State $validUpdate \gets \textsc{true}$
    \Else
      \State $validUpdate \gets \textsc{false}$ 
    \EndIf 
  \EndIf
...
\begin{algorithmic}[1]
  ...
  \State $\delta_{d} \gets \frac{\delta_{b}}{\tilde{\Pdt}}$ \Comment{Represents the maximum bank reserve growth during the ArbMint period.}
  \State $dexyAvailable \gets isCounterReset \textrm{ ? } \delta_{d} \textrm{ : } \tilde{d}$
  \State $validAmount \gets dexyMinted \le dexyAvailable$
  \If{$\lnot isCounterReset$}
    \State $T' \gets \tilde{t}$
    \State $validUpdate \gets \textsc{true}$
  \Else 
    \If{$T' \in [t + T_{\textrm{ARB}}, t + T_{\textrm{ARB}} + \tautx]$} \Comment{Checks new ArbMint period end time.}
      \State $validUpdate \gets \textsc{true}$
    \Else
      \State $validUpdate \gets \textsc{false}$ 
    \EndIf 
...
\begin{algorithmic}[1]
  ...
  \State $validDelay \gets t > \tilde{\tau} + \tau_{\textrm{ED}}$
  \State $validUpdate \gets t \le t' + \tautx$
  ...
  \If{$validAmounts \land validTimes$}
    ...
    \State $\sburn \gets (t', \tilde{t}_{\textrm{RD}}, d, \tilde{\tau}_{\textrm{ED}}, \tilde{\tau_{\textrm{RD}}})$
    \State \Return \textsc{true}
  \Else
    \State \Return \textsc{false}
  \EndIf
...
\begin{algorithmic}[1]
  ...
  \State $validDelay \gets t > \tilde{\tau} + \tau_{\textrm{RD}}$
  \State $validUpdate \gets t \le t' + \tautx$
  ...
  \If{$validAmounts \land validTimes$}
    \State $\slp \gets (B, D+d)$
    \State $\sburn \gets (\tilde{t}_{\textrm{ED}}, t', \tilde{d}-d, \tilde{\tau}_{\textrm{ED}}, \tilde{\tau_{\textrm{RD}}})$
    \State \Return \textsc{true}
  \Else
    \State \Return \textsc{false}
  \EndIf
