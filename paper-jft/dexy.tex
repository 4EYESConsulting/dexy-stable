...
\begin{abstract}
We consider a new stablecoin protocol design called Dexy. 
The protocol consists of only two assets: basecoins and stablecoins. The peg is maintained using 
two functional components: a reference market, in the form of a liquidity pool; 
and an algorithmic central bank, responsible for new stablecoin issuance and maintaining the stablecoin peg in the reference market.
\end{abstract}

\section{Introduction}
\label{sec:introduction}
Due to the usual volatility inherent with cryptocurrency prices, algorithmic stablecoins naturally arose as their extension. The volatility is resolved by pegging the stablecoin to an
asset whose price is considered to be stable over long periods of time, e.g. gold. 
...
The bank stabilizes the stablecoin value in the reference market, a liquidity pool, 
by injecting stablecoins when the stablecoin price is above the peg, or injecting basecoins from its reserves when the stablecoin price is below the peg. 
...
which will be injected back when the stablecoin price goes above the peg again. In some aspects, \dx{} could resemble Fei or Gyroscope \cite{Gyroscope}. 
...
A corresponding liquidity pool exists where stablecoins and basecoins can be traded. The peg price used by the bank is provided by an oracle, which is assumed to be trusted and reliable.
...
The reference market is implemented as an automated market maker (e.g. \cite{xu2021sok}). For simplicity a modified constant-product automated market maker (CP-AMM) is used, 
similar to ErgoDex \cite{ErgoDex} and UniSwap \cite{Uniswap}. This means that the liqudity pools holds a pair of basecoins and stablecoins such that the product of their amounts
remains invariant before and after a trade. Users who functions as liquidity providers can provide a pair of basecoins and stablecoins to obtain corresponding LP tokens.
These LP tokens can later be deposited to hopefully obtain the initial liquidity pair deposit along with accumulated fees from trading. A fee is charged to liqudity providers 
to prevent excessive liquidity withdrawals.
...
By depositing basecoins into the bank, users can mint stablecoins. The price provided by the bank will always equal the oracle price. A fee is charged so the bank reserves can increase faster.
