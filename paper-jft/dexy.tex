...
Central bank interventions are determined by the ratio of the liquidity pool price to the oracle price, $r$. 
...
ArbMint                               & $r > 1 + \epsilon_{\textrm{FREE}}$                                    & \multicolumn{1}{c|}{-}                                                                              \\ \hline
...
\begin{itemize}
  ...
  \item $\epsilon_{\textrm{RD}} < \epsilon_{\textrm{FEE}}$ defines the percentage where the release-dexy action will happen.
  ...
  \item $\alpha_{\textrm{FREE}} \in (0,1)$ is the percentage of the dexy liquidity in the liquidity pool that determines the max amount of dexy that can be minted from the bank during the free-mint period.
...
\begin{definition}[The Reference Price]
  The liquidity pool price, or the reference market price, is defined as the following, 
  and like the oracle price, has units of $\frac{\textrm{BASECOIN}}{\textrm{PEG ASSET}}$:
  \begin{equation}
    \Plp(\slp) = \frac{\slp.B}{\slp.D}
  \end{equation}
  
\end{definition}

\begin{definition}[The Reference Reserve Ratio]
    We call the ratio of the liquidity pool price to the oracle price the reference reserve ratio,
    for simplicity, because the formula resemble the typical formula defining a reserve ratio,
    namely the ratio of a reserve to its liabilities:
\begin{equation}
  \label{eq:r}
  r(\Pdt, \slp) = \frac{\Plp(\slp)}{\Pdt} = \frac{\slp.B}{\Pdt \slp.D}
\end{equation}
\end{definition}
