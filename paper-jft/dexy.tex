...
\begin{algorithmic}[1]
  ...
  \State $(\tilde{t}, tilde{d}) \gets \sfree$
  \State $\rr \gets \rr(\Pdt, \slp) $
  ...
  \State $\delta_{d} \gets \alpha_{\textrm{FREE}}\slp.D$ \Comment{Represents the maximum bank reserve growth during the FreeMint period.}
  ...
  \If{$validUpdate \land validRatio$}
    \State $\sbank \gets \sbank + (b, dexyMinted)$
    \State $\soracle \gets \soracle + (0, oracleFeeAmount)$
    \State $\sfree \gets (t', dexyAvailable - dexyMinted)$
  \Else
    \State $\sbank \gets \sbank$
    \State $\soracle \gets \soracle$
    \State $\sfree \gets \sfree$
  \EndIf
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
  ...
  \If{$validUpdate \land validDelay \land validRatio$}
    \State $\sbank \gets \sbank + (b, dexyMinted)$
    \State $\soracle \gets \soracle + (0, oracleFeeAmount)$
    \State $\sarb \gets (T', dexyAvailable - dexyMinted)$
  \Else
    \State $\sbank \gets \sbank$
    \State $\soracle \gets \soracle$
    \State $\sarb \gets \sarb$
...
\begin{algorithmic}[1]
  ...
  \If{$validAmounts \land validTimes$}
    \State $\sbank \gets \sbank - (b, 0)$
    \State $\slp \gets \slp + (b, 0)$
