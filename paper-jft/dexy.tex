...
The bank has two basic operations. It can mint new \stc{} tokens per request, using the trusted oracle's price, by accepting basecoins in its reserves. It also can intervene into markets by providing basecoins from the reserves when needed - when the price in the pool $\frac{e}{u}$ is significantly different from the price on external markets $p$ reported by the oracle.
...
a gold-pegged stablecoin named \dxg and a USD-pegged stablecoin named \dxusd.
