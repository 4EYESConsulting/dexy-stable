...
\begin{itemize}
  ...
  \item $\tau \in \mathbb{R}_{>0}\cup\{\infty\}$ is the time when $r > 1 + \epsilon_{\textrm{FREE}}$. Obviously, this can occur more than once as time goes on,
        each one corresponding to a potentially new arbitrage-mint period. 
\end{itemize}
...
\begin{algorithmic}[1]
  \Require $\soracle, \slp, \sbank, \sfree, \textrm{Basecoins } b, \textrm{Updated period end} t', \textrm{Current time } t$
