...
For the sake of clarity, we now defines separate state variables for the different bank actions, 
...
\begin{itemize}
  \item $t \in \mathbb{R}_{>0}$ is the time when the arb-mint period ends.
  \item $d \in \mathbb{R}_{\ge 0}$ is the remaining amount of dexy stablecoins that can be minted during the arb-mint period.
